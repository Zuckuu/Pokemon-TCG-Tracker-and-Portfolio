...
function createPagination() {
  if (cardData.length != 0) {
    var paginationEl = $("<ul>").addClass("pagination");
    var pageAmount = (totalResults - paginationIndex * 12 * 8) / 12;
    var leftArrowItem = $("<li>").addClass("page prevButton");
    ...
    for (i = 0; i < pageAmount && i < 8; i++) {
      var pageButton = $("<li>")
        .addClass("page pageButton")
        .data("index", i + paginationIndex * 8);
      if (pageIndex === i + paginationIndex * 8) {
        pageButton.addClass("active purple");
      }
      var pageLink = $("<a>").text(i + 1 + paginationIndex * 8);
      pageButton.append(pageLink);
      paginationEl.append(pageButton);
    }
    var rightArrowItem = $("<li>").addClass("page nextButton");
    ...
    $(".nextButton").on("click", function () {
      ...
      if ((pageIndex + 1) % 8 === 0) {
        ...
        pageIndex + 1 - paginationIndex * 8 <
        totalPageButtons.length
      ) {
        pageIndex++;
      }
      searchCards();
...
function pullCardData() {
  ...
  $.ajax({
    url: finalSearchUrl,
    method: "GET",
    headers: { "X-Api-Key": "2d9df781-38e6-4447-8b8c-5760c4c5b244" },
...
function searchCards() {
  ...
  var cardsRow = $("<div>").addClass("cardsRow");
  for (i = pageModifier; i < pageModifier + 12; i++) {
    if (cardData[i] != undefined) {
      var divider = $("<div>").addClass("hide-on-small-and-down");
      var cardContainer = $("<div>").addClass("cardContainer col s12 m3 l3");
      var imageContainer = $("<div>").addClass("img-container");;
      var textContainer = $("<div>").addClass(
        "textContainer halign-wrapper center"
      );
      var favButton = $("<a>")
        .addClass("btn-floating favbutton page")
        .data("card", cardData[i].id);
      for (l = 0; l < favoritesList.length; l++) {
        if (favoritesList[l].id === cardData[i].id) {
          favButton.addClass("green");
        }
      }
      if (favoritesList.length == 0) {
        favButton.addClass("grey");
      }
      var favIcon = $("<span>").addClass("addPortbtn").text("Add to Portfolio");
      ...
      var imageEl = $("<img>")
        .attr({ src: cardData[i].images.large, width: 250 })
        .addClass("responsive-img");
      var id = cardData[i].id;
      var imageLink = $("<a>")
        .attr({ href: `./cardpage.html?id=${id}`, target: "_blank" })
        .append(imageEl);
      ...
      if (i === 249) {
        if (favoritesShown != true) {
          var maxWarning = $("<h4>")
            .text(
              "Max results reached - Narrow your search criteria to see more!"
            )
            .addClass("col s12 center push-l1");
